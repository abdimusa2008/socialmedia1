...
#        self.w.grid(row=7,column=3, padx=5, pady=3) 
...
########################################################
        ...
        daylabel.grid(row=9,column=0, padx=5, pady=3)

        self.day = Entry(self, width=5)
        self.day.grid(row=10,column=0, padx=5, pady=3)
        
        monthlabel=Label(self,text='Month')
        monthlabel.grid(row=11,column=0, padx=5, pady=3)
        
        self.month = Entry(self, width=5)
        self.month.grid(row=12,column=0, padx=5, pady=3)
        ...
        self.w.grid(row=13,column=0, padx=5, pady=3)

        self.f = Button(self, text='Interractions',command=self.get_interractions)
        self.f.grid(row=13,column=3, padx=5, pady=3)
...
### import model
        ...
        for tweet in tweepy.Cursor(api.search,
                               ...
                               since='2018-03-12',
                               until='2018-03-18',
                               lang=tso.set_language('so')).items(30):
            # lang='en').items(6):
...
#####################################################################################################################################
    ...
    def get_interractions(self):
        ...
        #vario = self.variable.get()
        
        #username = "somaliya_cusub"# sys.argv[1]
        # 2018, 2, 14, 0, 0, 0
        #   t.Year(), t.Month(), t.Day(),t.Hour(), t.Minute(), t.Second())
        
        startDate = datetime.datetime(2018, 3, 12, 0, 0, 0)
        endDate =   datetime.datetime(2018, 3, 19, 11, 59, 59)
#        ############################################################################        
#        for i in range(100):
#            print(i)
#            
...
#        ############################################################################
        ...
        #print("value is:" + vario)
        for status in tweepy.Cursor(status.user_timeline, username).items(100):
...
#                    
#                    #hito = hito.encode('utf-8')
#                    # 3 replies 9 retweets 22 likes 1 
#                    # 1 reply 1 retweet 8 likes 1 reply 0 retweets 1 like 0 replies 0 retweets 1 like 
#
#                    print(hito)
#                    hito = hito.replace('replies ', '')
#                    hito = hito.replace('retweets ', '')
#                    hito = hito.replace('likes ', '')
#                    hito = hito.replace('reply ', '')
#                    hito = hito.replace('retweet ', '')
#                    hito = hito.replace('like ', '')
#                    print(hito)
#                    hito = hito.split(" ")
#                    if " " in hito[0]:
#                        print("0")
...
#                        print(hito[2])
                    ...
                    # https://stackoverflow.com/questions/41359338/regular-expression-for-arabic-in-python
                    # re.search(r"(?<=اسمي)\s\D\w+", H, re.U)
                    if (status.created_at < endDate and status.created_at > startDate):
                        print(status.created_at)
                        print(hito[:28])
                    
                    
#                    m = re.match(r"(\w+) (\w+) (\w+) (\w+) (\w+) (\w+)", hito,  re.U)
#                    #print(status.created_at)
#                    #print("=======")
#                    
#                    print(m.group(0))
